        ...
        if (this.validity == null) {
            ...
            try {
                ...
                for (String facetField : queryResults.getFacetResults().keySet()) {
                    ...
                    List<DiscoverResult.FacetResult> facetValues = queryResults.getFacetResults().get(facetField);
...
//            facetFields.addAll(this.queryResults.getFacetDates());
    ...
    private void addBrowseJumpNavigation(Division div, SearchFilterParam browseParams, Request request)
        ...
        Division jump = div.addInteractiveDivision("filter-navigation", action,
                Division.METHOD_POST, "secondary navigation britch");
        ...
        for (String filterQuery : filterQueries) {
            jumpc.addHidden("fq").setValue(filterQuery);
        }
        ...
        if(!browseParams.getFacetField().endsWith(".year")){
            ...
            org.dspace.app.xmlui.wing.element.List jumpList3 = jumpc.addList("jump-list3", org.dspace.app.xmlui.wing.element.List.TYPE_SIMPLE, "alphabet col-md-1");
            String linkNumUrl = basicUrl + "&" +  SearchFilterParam.STARTS_WITH +  "=" + "0";
            jumpList3.addItemXref(linkNumUrl, "0-9");

            //TODO: put this back !
//            jumpList.addItemXref(generateURL("browse", letterQuery), "0-9");

            org.dspace.app.xmlui.wing.element.List jumpList2 = jumpc.addList("jump-list2", org.dspace.app.xmlui.wing.element.List.TYPE_SIMPLE, "alphabet col-md-5");
            ...
            for (char qq = 'Σ'; qq <= 'Ω'; qq++) {
                String linkUrl = basicUrl + "&" +  SearchFilterParam.STARTS_WITH +  "=" + Character.toString(qq).toLowerCase();
                jumpList2.addItemXref(linkUrl, Character
                        .toString(qq));
            }

              // Create a clickable list of the alphabet
            org.dspace.app.xmlui.wing.element.List jumpList = jumpc.addList("jump-list", org.dspace.app.xmlui.wing.element.List.TYPE_SIMPLE, "alphabet col-md-6");
            ...
            Para jumpForm = jumpc.addPara("lab","");
...
//        }
        ...
        Message displayedMsg=message(displayedValue);
                                        // modified by imc
                                        if (facetField.equals("type"))
                                        {
                                            displayedMsg=message("view.heal.type."+displayedValue).parameterize(" (" + value.getCount() + ")") ;
                                        }
