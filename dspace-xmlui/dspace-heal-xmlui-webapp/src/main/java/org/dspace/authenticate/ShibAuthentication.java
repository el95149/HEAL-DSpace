...
public class ShibAuthentication implements AuthenticationMethod
{
	...
	public int authenticate(Context context, String username, String password,
		...
		if (log.isDebugEnabled()) {
			log.debug("Starting Shibboleth Authentication [HEAL]");
	...
	private EPerson registerNewEPerson(Context context, HttpServletRequest request) throws SQLException, AuthorizeException {
        ...
        fname = convertToUTF(fname);
        lname = convertToUTF(lname);
    ...
    public String convertToUTF(String s) {

        if (!StringUtils.isEmpty(s)) {
            try {
                s = new String(s.getBytes("ISO-8859-1"), "UTF-8");
            } catch (UnsupportedEncodingException ex) {
                log.warn("Failed to reconvert shibboleth attribute (" + s + ").", ex);
            }
        }
