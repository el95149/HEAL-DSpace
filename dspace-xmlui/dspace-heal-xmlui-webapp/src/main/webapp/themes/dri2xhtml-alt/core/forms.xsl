...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template match="dri:instance" mode="interpreted">
		...
		<!-- modified by aanagnostopoulos: Language value is displayed, if available -->
        <span class="ds-interpreted-field">
        <xsl:choose>
        ...
        <xsl:if test="dri:value[@type='lang']!=''">
         (<xsl:value-of select="dri:value[@type='lang']"/>)
         </xsl:if>
         </span>
         <!-- END aanagnostopoulos -->
    </xsl:template>
    ...
    <xsl:template match="dri:field" mode="compositeComponent">
        <xsl:choose>
                ...
                <xsl:otherwise>
                <xsl:choose>
                <xsl:when test="dri:params[@i18nable = 'yes']|../dri:params[@i18nable = 'yes']">
                            <xsl:apply-templates select="." mode="normalField"/>
                </xsl:when>
                <xsl:otherwise>
	...
	<xsl:template match="dri:field|dri:instance" mode="i18n-field">
		...
		<label class="ds-composite-component">
			<select class="ds-select-field ds-select-lang">
				<xsl:attribute name="name"><xsl:value-of select="$name"/></xsl:attribute>
				<!-- option list -->
				<xsl:apply-templates select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='page'][@qualifier='supported_locale']">
	...
	<xsl:template match="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='page'][@qualifier='supported_locale']">
		<xsl:param name="lang"/>
		...
		<option>
			...
			<xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='supported_locale'][@qualifier=$locale]"/>
		</option>
	</xsl:template>
    <!-- END aanagnostopoulos -->
