...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
            ...
            <xsl:if test="contains(dri:params/@operations,'add')">
                <i18n:choose>
                    ...
                    <!-- Add buttons should be named "submit_[field]_add" so that we can ignore errors from required fields when simply adding new values-->
                       <input type="submit" value="Προσθήκη" name="{concat('submit_',@n,'_add')}" class="ds-button-field ds-add-button  btn btn-primary">
                    ...
                    <i18n:otherwise>
                        <input type="submit" value="Add" name="{concat('submit_',@n,'_add')}" class="ds-button-field ds-add-button  btn btn-primary">
                            <!-- Make invisible if we have choice-lookup operation that provides its own Add. -->
                            <xsl:if test="dri:params/@choicesPresentation = 'lookup'">
                                <xsl:attribute name="style">
                                    <xsl:text>display:none;</xsl:text>
            ...
            <xsl:if test="dri:instance or dri:field/dri:instance">
                <div class="ds-previous-values">
                    ...
                    <xsl:if test="contains(dri:params/@operations,'delete') and (dri:instance or dri:field/dri:instance)">
                        ...
                        <i18n:choose>
                            <i18n:when locale="el">
                                <input type="submit" value="Αφαίρεση" name="{concat('submit_',@n,'_delete')}" class="ds-button-field ds-delete-button  btn btn-danger" />
                            </i18n:when>
                            <i18n:otherwise>
                                <input type="submit" value="Remove selected" name="{concat('submit_',@n,'_delete')}" class="ds-button-field ds-delete-button  btn btn-danger" />
                            </i18n:otherwise>
                        </i18n:choose>
                    </xsl:if>
    ...
    <xsl:template match="dri:field">
        ...
        <xsl:if test="contains(dri:params/@operations,'add')">
            ...
            <i18n:choose>
                <i18n:when locale="el">
                    <input type="submit" value="Προσθήκη" name="{concat('submit_',@n,'_add')}" class="ds-button-field ds-add-button">
    ...
    <!--modified to differentiate between selects with and without attribute 'multiple'-->
    <xsl:template name="fieldAttributes">
        <xsl:call-template name="standardAttributes">
            <xsl:with-param name="class">
                <xsl:text>ds-</xsl:text><xsl:value-of select="@type"/>
                <xsl:text>-field </xsl:text>
                <xsl:if test="@type!='button' and @type!='submit'">
                    <!--  <xsl:variable name="someNode" select="//dri:div/dri:div[@id='aspect.artifactbrowser.ConfigurableBrowse.div.browse-navigation']" />
                     <xsl:if test="current()/ancestor::*[generate-id() = generate-id($someNode)]">-->
                     <xsl:text>  input  </xsl:text>
                    <!-- </xsl:if>-->
                </xsl:if>
                <xsl:if test="@type='button' or @type='submit'">
    ...
    <xsl:template match="dri:field[dri:field/dri:instance | dri:params/@operations]" priority="2">
        <xsl:variable name="qualdrop" select="@rend='submit-qualdrop'"/>
    ...
    <!--<xsl:template match="dri:list[@id='aspect.eperson.PasswordLogin.list.password-login']">-->
        <!--<xsl:call-template name="standardAttributes">-->
            <!--<xsl:with-param name="class">form-horizontal</xsl:with-param>-->
        <!--</xsl:call-template>-->
        <!--<xsl:apply-templates/>-->
    <!--</xsl:template>-->

    <!--<xsl:template match="dri:list[@id='aspect.eperson.PasswordLogin.list.password-login']//dri:field/dri:label/*">-->
        <!--<xsl:call-template name="standardAttributes">-->
            <!--<xsl:with-param name="class">control-label col-md-3</xsl:with-param>-->
        <!--</xsl:call-template>-->
        <!--<xsl:copy-of select="."/>-->
    <!--</xsl:template>-->

    <!--<xsl:template match="dri:field[@id='aspect.eperson.PasswordLogin.field.login_email']">-->
    ...
    <!--<xsl:template match="dri:field[@id='aspect.eperson.PasswordLogin.field.login_password']">-->
        <!--<xsl:attribute name="class">col-md-9</xsl:attribute>-->
        <!--<input>-->
            <!--<xsl:copy-of select="@*"/>-->
            <!--<xsl:apply-templates select="dri:field"/>-->
    ...
    <xsl:template match="dri:list[@id='aspect.eperson.PasswordLogin.list.password-login']//dri:field[@type='text' or @type='password']/*">
        <xsl:call-template name="standardAttributes">
    ...
    <!-- Feedback Form -->
    <xsl:template match="dri:list[@id='aspect.artifactbrowser.FeedbackForm.list.form']//dri:field[@type='text' or @type='textarea']/*">
        <xsl:call-template name="standardAttributes">
            <xsl:with-param name="class">form-control</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
