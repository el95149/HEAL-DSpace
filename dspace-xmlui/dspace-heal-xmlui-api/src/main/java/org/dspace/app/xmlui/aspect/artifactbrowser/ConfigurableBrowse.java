 ...
 */
public class ConfigurableBrowse extends AbstractDSpaceTransformer implements CacheableProcessingComponent {
        ...
        if (itemsTotal > 0)
        {
            ...
            else    // browsing a list of unique metadata entries
            {
                ...
                for (String[] singleEntry : browseInfo.getStringResults())
                {
                    ...
                    //log.error("type equals "+type);
                    String val=singleEntry[0];
                    // modified by imc
                    if (type.equals("type"))
                    {
                        Message msg=message("view.heal.type."+val).parameterize("") ;
                        //val= val+"-"+I18nUtil.getMessage(,context.getCurrentLocale());
                         //val= val+"-"+I18nUtil.getMessage("heal.type.studentID",context.getCurrentLocale());
            ...
            throws WingException
    {
        ...
        Division jump = div.addInteractiveDivision("browse-navigation", BROWSE_URL_BASE,
                Division.METHOD_POST, "date navigation");
        Division jumpc = jump.addDivision("browse-navigation-well", "well");

        // Add all the query parameters as hidden fields on the form
        for (Map.Entry<String, String> param : queryParamsPOST.entrySet())
        {
            jumpc.addHidden(param.getKey()).setValue(param.getValue());
        }
        ...
        if (isSortedByDate(info))
        {
            ...
            Division jumpe=jumpc.addDivision("jump1","row");
            Para jumpForm = jumpe.addPara("lab","col-md-4");

            // Create a select list to choose a month
            jumpForm.addContent(T_jump_select);

            Para jumpFormY = jumpe.addPara("lab","col-md-4");
            Select month = jumpFormY.addSelect(BrowseParams.MONTH,"input input-sm");
            ...
            Para jumpFormM = jumpe.addPara("lab","col-md-4");
            Select year = jumpFormM.addSelect(BrowseParams.YEAR,"input input-sm");
            ...
            Division jumpd=jumpc.addDivision("jump2","row");
            // Create a free text entry box for the year
            jumpForm = jumpd.addPara();
        ...
        else
        {
            ...
            List jumpList3 = jumpc.addList("jump-list3", List.TYPE_SIMPLE, "alphabet col-md-1");
            letterQuery.put(BrowseParams.STARTS_WITH, "0");
            jumpList3.addItemXref(super.generateURL(BROWSE_URL_BASE, letterQuery), "0-9");

            List jumpList2 = jumpc.addList("jump-list2", List.TYPE_SIMPLE, "alphabet col-md-5");
            for (char vv = 'Α'; vv <= 'Ρ'; vv++) {
                letterQuery.put(BrowseParams.STARTS_WITH, Character.toString(vv));
                jumpList2.addItemXref(super.generateURL(BROWSE_URL_BASE, letterQuery), Character.toString(vv));
            }
            for (char qq = 'Σ'; qq <= 'Ω'; qq++) {
                letterQuery.put(BrowseParams.STARTS_WITH, Character.toString(qq));
                jumpList2.addItemXref(super.generateURL(BROWSE_URL_BASE, letterQuery), Character.toString(qq));
            }

            // Create a clickable list of the alphabet
            List jumpList = jumpc.addList("jump-list", List.TYPE_SIMPLE, "alphabet col-md-6");
